...
while [ $c -le $jobs ]; do 
    ...
    sleep .5
    sfdx force:apex:execute -f test_apex/runPLDSync.apex -u test-17zksac7khsq@example.com  >> outSyncPLDA.log &
    sleep .5
    sfdx force:apex:execute -f test_apex/runSFSDKAsync.apex -u test-17zksac7khsq@example.com >> outAsyncSFSDK.log &
    sleep .5
    sfdx force:apex:execute -f test_apex/runSFSDKSync.apex -u test-17zksac7khsq@example.com >> outSyncSFSDK.log &
    sleep .5
