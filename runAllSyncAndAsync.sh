...
rm *.log
while [ $c -le $jobs ]; do 
    sfdx force:apex:execute -f test_apex/runPLDAsync.apex -u test-6dexhfosdeif@example.com >> outAsyncPLDA.log &
    sleep .5
    sfdx force:apex:execute -f test_apex/runPLDSync.apex -u test-6dexhfosdeif@example.com  >> outSyncPLDA.log &
    sleep .5
    sfdx force:apex:execute -f test_apex/runSFSDKAsync.apex -u test-6dexhfosdeif@example.com >> outAsyncSFSDK.log &
    sleep .5
